...
describe('remote-ffprobe', () => {
  it('gives ffprobe info for a streamable video', async () => {
    ...
    const info = await remoteffprobe('https://testing.com/streamable.mp4', { ffprobePath });
  ...
  it('gives ffprobe info for a non-streamable video', async () => {
    ...
    const info = await remoteffprobe('https://testing.com/video.mp4', { ffprobePath });
  ...
  it('errors cleanly on 404', async () => {
    ...
    const probe = remoteffprobe('https://testing.com/404.mp4', { ffprobePath });
  ...
  it('should cleanly fail on ETIMEDOUT', async () => {
    ...
    const probe = remoteffprobe('https://testing.com/timeout.mp4', { timeout: 100, ffprobePath });
    await expect(probe).to.eventually.be.rejectedWith(Error);
  });
  it('should fail cleanly when it\'s actually not a video', async () => {
    ...
    const probe = remoteffprobe('https://testing.com/somepage.html', { ffprobePath });
    await expect(probe).to.eventually.be.rejectedWith(Error);
    nock.cleanAll();
  });
});
